...
func has_saved_games():
	...
	var table_list = self.fetch_all_tables(db_file)
	
	return table_list
	
func save_data(data_dict):
	
	var statement = self.get_insert_statement()
	var table_name = self.get_table_name();
	var table_query = self.get_table_query(table_name);
	var param_array = []
	
	#adujust query to the size of the data (more than 2 rows to save)
	
	for i in range(1,data_dict.size()):
		table_query += "('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"#. %s');"
	table_query += ";"
	...
	var query = statement % param_array.push_front(table_name)
	
	return self.save(db_file,table_query,table_name,query)
