...
func _ready():
	...
	self.get_orbitMenu_ready()
	
	center_container.add_child(grid)
	self.add_child(center_container)
	message_label.text = "Choose the Planet for the Center of the Orbit."
	pass
